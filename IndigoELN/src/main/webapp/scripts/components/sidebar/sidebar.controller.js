        ...
        Principal.identity()
            .then(function (user) {
                ...
                var promiseToggleNotebook = function (agent, project, file) {
                    return agent.query({projectId: project.id}, function (result) {
                        project.children = result;
                        project.isOpen = true;
                        saveInStorage({id: project.id, isOpen: true}, file);
                    });
                };

                var promiseToggleExperiment = function (agent, notebook, project, file, save) {
                    return agent.query({notebookId: notebook.id, projectId: project.id}, function (result) {
                ...
                var toggleProjects = function (parent, needAll, fun, file) {
                    ...
                    if (!parent.projects) {
                        ...
                        var data = {id: initialProject.id, isOpen: false};
                        localStorageService.set(file, JSON.stringify(data));
                ...
                var toggleNotebooks = function (project, needAll, fun, file) {
                    ...
                    if (project.isOpen) {
                        ...
                        saveInStorage({id: project.id, isOpen: false}, file);
                ...
                $scope.restoreMyTabs = function (scope) {
                    var firstLevelFile, secondLevelFile, thirdLevelFile;
                    var firstLevelFun, secondLevelFun, thirdLevelFun;
                    var tabsOnScope;
                    var needAll;
                    if (scope === MYPROJECTS) {
                ...
                $scope.toggleAdministration = function () {
                    ...
                    var data = {id: 'adm', isOpen: $scope.adminToggled};
