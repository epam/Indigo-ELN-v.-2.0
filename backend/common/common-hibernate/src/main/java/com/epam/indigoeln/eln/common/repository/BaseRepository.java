...
public abstract class BaseRepository<E extends IdentifiableEntity> implements PanacheRepositoryBase<E, UUID> {
    ...
    protected List<E> doFind(Conditions conditions, Paging paging, Sort sort, EntityGraph<?> entityGraph) {
        PanacheQuery<E> query = doCreateQuery(conditions, paging, sort, entityGraph);
        return query.list();
    }
    ...
    protected E doLoad(Conditions conditions, EntityGraph<?> entityGraph) {
        PanacheQuery<E> query = doCreateQuery(conditions, null, null, entityGraph);
        return query
                .withHint("jakarta.persistence.loadgraph", entityGraph)
                .singleResultOptional()
                .orElseThrow(() -> new jakarta.persistence.EntityNotFoundException(entityType + " not found"));
    }
    ...
    private PanacheQuery<E> doCreateQuery(Conditions conditions, @Nullable Paging paging, @Nullable Sort sort, @Nullable EntityGraph<?> entityGraph) {
