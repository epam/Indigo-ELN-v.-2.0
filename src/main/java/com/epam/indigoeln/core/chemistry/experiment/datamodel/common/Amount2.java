 ...
 * 1.03 cm (three significant figures). 3) Zeros within a number are always significant. Both 4308 and 40.05
 * contain four significant figures 4) Zeros to the left of the first nonzero digits in a number are not significant;
 * they merely indicate the position of the decimal point--0.02 g (one significant figure);
 * 0.0026 cm (two significant figures). 5) When a number ends in zeros that are to the right of the decimal point,
 * they are significant--0.0200 g (three significant figures); 3.0 cm (two significant figures).
 */
public class Amount2 extends CeNAbstractModel implements DeepClone, DeepCopy {
     ...
     *
     * @param toUnit Unit
     * @return the value as if the amount object were set to that unit
     */
     ...
     * If the string is "" the default value is used. The default value is initialized as 0.000;
     * isCalc indicates the status of the amount object whether it is a calculated quantity or is a user set quantity.
     *
    ...
    /**
     * Sets value that the amount object will use when created without a value, and used when a
     * null string is entered in setValue as the null string indicates the defaults are to be imposed.
     *
     * @param val -
    ...
    /**
     * Standard figures for display come from the unit itself. This can be overwritten by adding a user preference
     * for figures to be displayed Ultimately both are overridden should the user enter a value by hand.
     * Then the Sig figs of that value are what are important to display.
     *
