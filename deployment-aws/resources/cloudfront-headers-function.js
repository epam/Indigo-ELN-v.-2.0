...
function handler(event) {
    ...
    if (uri.match(STATIC_RESOURCES) || uri.match(FONT_RESOURCES) || uri.match(KETCHER_STATIC_RESOURCES)) {
        ...
        let csp = "default-src 'self'; object-src 'none'; frame-ancestors 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; connect-src 'self' https://cognito-idp.us-east-1.amazonaws.com; font-src 'self' https://fonts.gstatic.com; worker-src 'self' blob:; media-src 'self' data:; img-src 'self' blob: data:;";
