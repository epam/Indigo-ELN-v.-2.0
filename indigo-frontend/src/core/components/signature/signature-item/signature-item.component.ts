...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, CardComponent, MatButtonModule, MatIconModule, MatMenuModule, AvatarComponent, RouterLink],
...
export class SignatureItemComponent {
  ...
  download() {
    this.service
      .request<Blob>('get', `/signature/experiments/${this.experiment.id}/download`, undefined, {
        responseType: 'blob',
      })
      .pipe(
      ...
      .subscribe({
        ...
        next: (blob: Blob | null) => downloadBlob(blob, `Experiment ${this.experiment.name}.pdf`),
      });
  }

  approve() {
    this.service
      .request<ExperimentForSignature>('post', `experiments/${this.experiment.id}/workflow/approve`)
      .pipe(
        catchError((err) => {
  ...
  reject() {
    this.service
      .request<ExperimentForSignature>('post', `experiments/${this.experiment.id}/workflow/reject`)
      .pipe(
        catchError((err) => {
